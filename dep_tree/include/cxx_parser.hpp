...
void parse_cxx(boost::filesystem::path const& path, dep_tree& root, std::vector< boost::filesystem::path > const& include_dirs = std::vector< boost::filesystem::path >(), bool create_reverse_dependencies = true);
